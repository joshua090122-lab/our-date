...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (password.length < 8 || password.length > 256 || !/^[A-Za-z0-9]{32,128}$/.test(gateway)) {
      return send(res, 503, {message:'앱 입장 준비가 아직 완료되지 않았어요. 배포 안내서의 비밀번호 설정을 확인해 주세요.', code:'SETUP_REQUIRED'});
    }
